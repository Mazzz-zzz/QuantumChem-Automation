...
5. Extracts key frames (reactants, transition state, products) for static analysis
"""
...
def create_gaussian_input_file(molecule, reaction_type, timestep, atoms, output_dir, 
                              step_num, method="wb97xd", basis="6-311++G(d,p)"):
...
# ============== STATIC FRAMES EXTRACTION FUNCTIONS ==============

def extract_key_frames(xyz_file):
    """Extract the three key frames (reactants, transition state, products) from an XYZ trajectory."""
    frames = read_xyz_frames(xyz_file)
    ...
    # For IRC trajectories, the key frames are:
    # 1. Reactants: First frame (end of reverse IRC)
    # 2. Transition state: Middle frame
    # 3. Products: Last frame (end of forward IRC)
    
    reactants = frames[0]
    transition_state = frames[len(frames) // 2]
    products = frames[-1]
...
def create_static_orbital_inputs(xyz_files, output_dir="./static_orbital_results", 
                                method="wb97xd", basis="6-311++G(d,p)"):
    """Create input files for the three key frames (reactants, TS, products) of each reaction."""
    # Create output directory
    os.makedirs(output_dir, exist_ok=True)
    
    # Store information about all generated static inputs
    all_static_inputs = {}
    
    print("\n=== Extracting static frames for key reaction points ===")
    for i, xyz_file in enumerate(xyz_files):
        xyz_path = Path(xyz_file)
        print(f"Processing file {i+1}/{len(xyz_files)}: {xyz_path}")
        
        # Parse molecule name from file path
        filename = xyz_path.name
        if "_combined.xyz" in filename:
            molecule = filename.split("_combined.xyz")[0]
        else:
        ...
        key_frames = extract_key_frames(xyz_file)
        if not key_frames:
            print(f"  - Skipping {xyz_path} due to error in frame extraction")
            continue
        
        # Create directory for this molecule
        mol_dir = Path(output_dir) / molecule
        mol_dir.mkdir(parents=True, exist_ok=True)
        
        # Store input files for this molecule
        molecule_inputs = {}
        
        # Create Gaussian input files for each key frame
        for frame_type, (timestep, atoms) in key_frames.items():
            # Create input files for this frame type
            gjf_file = create_gaussian_input_file(
                molecule, frame_type, timestep, atoms, output_dir, 
                0 if frame_type == "reactants" else (1 if frame_type == "transition_state" else 2),
                method, basis
            )
            
            # Add to the list of inputs
            molecule_inputs[frame_type] = {
                "input_file": gjf_file,
                "molecule": molecule,
                "original_timestep": timestep
            }
            
            # Also save a separate XYZ file for this specific frame
            xyz_output = mol_dir / f"{molecule}_{frame_type}.xyz"
            with open(xyz_output, 'w') as f:
                f.write(f"{len(atoms)}\n")
                f.write(f"{molecule} {frame_type} - {timestep}\n")
                for symbol, x, y, z in atoms:
                    f.write(f"{symbol} {x:12.6f} {y:12.6f} {z:12.6f}\n")
            print(f"  - Saved {frame_type} frame to {xyz_output}")
        
        # Add this molecule's inputs to the main dictionary
        if molecule_inputs:
            all_static_inputs[molecule] = molecule_inputs
        
        print(f"  - Created static frame inputs for {molecule}")
        print("----------------------------------------")
    
    # Write the summary JSON file
    summary_file = Path(output_dir) / "static_input_summary.json"
    with open(summary_file, 'w') as f:
        json.dump(all_static_inputs, f, indent=2)
    
    print(f"\nSummary of static frame inputs saved to: {summary_file}")
    return all_static_inputs


def main():
    parser = argparse.ArgumentParser(description="Extract IRC geometries and generate orbital calculation inputs")
    parser.add_argument("--irc-dir", default="../AUTOMATION-CENTER/kinbot_jobs/CF3COOH-HIR",
    ...
    parser.add_argument("--static-dir", default="./static_orbital_results",
                        help="Directory to store static frame results (reactants, TS, products) (default: ./static_orbital_results)")
    ...
    parser.add_argument("--skip-static", action="store_true",
                        help="Skip static frame extraction (reactants, TS, products)")
    ...
    if xyz_files:
        ...
        # STEP 3: Extract static frames (reactants, TS, products)
        if not args.skip_static:
            print("\n=== STEP 3: Extracting static frames (reactants, TS, products) ===")
            all_static_inputs = create_static_orbital_inputs(
                xyz_files,
                output_dir=args.static_dir,
                method=args.method,
                basis=args.basis
            )
            
            # Count static inputs (3 per molecule: reactants, TS, products)
            total_static_inputs = sum(len(molecule_data) 
                                   for molecule_data in all_static_inputs.values())
            
            print(f"\nGenerated {total_static_inputs} static frame input files in: {args.static_dir}")
        
        print(f"\nNext steps:")
        print(f"1. Generated {total_inputs} Gaussian input files in: {args.output_dir}")
        if not args.skip_static:
            print(f"2. Static frame inputs (reactants, TS, products) in: {args.static_dir}")
        print(f"3. Use the submit_orbital_calculations.sh script to run the calculations:")
        print(f"   $ sbatch submit_orbital_calculations.sh {args.output_dir}")
        if not args.skip_static:
            print(f"   $ sbatch submit_orbital_calculations.sh {args.static_dir}")
