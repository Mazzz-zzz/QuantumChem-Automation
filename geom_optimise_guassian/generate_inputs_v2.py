...
def create_gaussian_input(mol, name, charge, multiplicity, output_dir="gaussian_projects"):
    ...
    with open(output_path, 'w') as f:
        f.write(f"%chk={name}.chk\n")  # Use relative path for checkpoint file
        # Header
        f.write("%mem=10GB\n")
        f.write("%nprocshared=8\n")
        #remove geom=connectivity
        f.write("# opt=(maxcyc=999,noeigen) freq m062x/def2tzvp int=ultrafine scf=(tight,xqc)\n\n")
...
def create_reanalysis_input(name, charge, multiplicity, temp, output_dir="gaussian_projects"):
    """Create a Gaussian input file that re-uses the Hessian via ReadFC"""
    gjf_path = Path(output_dir) / f"{name}_{int(temp)}K.gjf"
    with open(gjf_path, 'w') as f:
        # Link 0: reuse the checkpoint
        f.write(f"%chk={name}.chk\n")  # Use relative path for checkpoint file
        f.write("%NoSave\n")
        # Route: read geometry, orbitals, and force constants
        f.write("# m062x/def2tzvp geom=check guess=read "
                "freq=readfc "
                "int=ultrafine scf=(tight,xqc)\n\n")
        f.write(f"{name} thermochemistry reanalysis at {temp} K\n\n")
        # Charge/Multiplicity
        f.write(f"{charge} {multiplicity}\n")
        # Temperature, pressure, scale
        f.write("\n")
        f.write(f"{float(temp)} 1.0 1.0\n\n")
    print(f"  • Wrote reanalysis input: {gjf_path}")

def create_frequency_input(mol, name, charge, multiplicity, temp, output_dir="gaussian_projects"):
    """Create a fresh Gaussian frequency input file at a given temperature."""
    gjf_path = Path(output_dir) / f"{name}_{int(temp)}K.gjf"
    chk_path = f"{name}_{int(temp)}K.chk"
    with open(gjf_path, 'w') as f:
        # New checkpoint for this frequency run
        f.write(f"%chk={chk_path}\n")
        f.write("%mem=10GB\n")
        f.write("%nprocshared=8\n")
        # Route: frequency calculation only with temperature specified
        f.write(f"# freq m062x/def2tzvp int=ultrafine scf=(tight,xqc) Temp={int(temp)}\n\n")
        f.write(f"{name} frequency calculation at {temp} K\n\n")
        # Charge and multiplicity
        f.write(f"{charge} {multiplicity}\n")
        # Coordinates from RDKit molecule
        conf = mol.GetConformer()
        for i, atom in enumerate(mol.GetAtoms()):
            pos = conf.GetAtomPosition(i)
            f.write(f"{atom.GetSymbol():2s}    {pos.x:10.6f}    {pos.y:10.6f}    {pos.z:10.6f}\n")
        ...
        # Temperature, pressure, scale
        f.write(f"{float(temp)} 1.0 1.0\n\n")
    print(f"  • Wrote frequency input: {gjf_path}")
...
def main():
    ...
    # Define temperatures for reanalysis
    temps = [200, 400, 600, 800, 1000, 1200]
    ...
    for name, specs in molecules.items():
        try:
            ...
            # Create Gaussian input file for optimization
            ...
            print(f"Created optimization input for {name}")
            
            # Create fresh frequency inputs at each temperature
            for T in temps:
